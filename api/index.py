...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path == "/api/format_sheet":
            creds_json = os.environ.get("GOOGLE_SHEETS_CREDENTIALS")
            spreadsheet_id = os.environ.get("GOOGLE_SPREADSHEET_ID")
            
            if not creds_json or not spreadsheet_id:
                self._send_json(400, {"status": "error", "message": "Credentials missing"})
                return
            ...
            try:
                ...
                # Format Reviews sheet (Sheet1)
                worksheet = sh.get_worksheet(0)
                worksheet.update_title("Reviews")
                
                headers = ["Timestamp", "Client ID", "Rating", "Message", "Phone Number", "Status"]
                ...
                sheet_id = worksheet._properties['id']
                
                # Format Leads sheet
                try:
                    leads_sheet = sh.worksheet("Leads")
                except Exception:
                    leads_sheet = sh.add_worksheet(title="Leads", rows="1000", cols="6")
                
                leads_sheet.update('A1:F1', [headers])
                leads_sheet.format('A1:F1', {
                    'backgroundColor': {'red': 0.90, 'green': 0.92, 'blue': 0.95},
                    'textFormat': {'bold': True, 'fontSize': 10, 'fontFamily': 'Arial'},
                    'horizontalAlignment': 'CENTER'
                })
                ...
                # Apply conditional formatting rules to Reviews sheet
                requests = [
                    {
                ...
                self._send_json(200, {"status": "success", "message": "Google Sheet styled and structured successfully"})
            except Exception as e:
                self._send_json(500, {"status": "error", "message": f"Formatting failed: {str(e)}"})
            return
